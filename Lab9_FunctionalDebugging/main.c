...
void PortF_Init(void){ volatile unsigned long delay;
  ...
  GPIO_PORTF_DEN_R = 0x1F;          // 7) enable digital I/O on PF4-0
	GPIO_PORTF_DATA_R &= ~0x02;       // 8) PF1 output should begin low
}
...
int main(void){  
	...
	unsigned int change_flag;        // 1 if change; 0 if no change
	unsigned int pressed_down_flag = 0;  // 1 if input pressed; 0 if not pressed
  ...
  while(1){
    Led = GPIO_PORTF_DATA_R;   // read previous
		change_flag = 0;           // reset the change flag
				
		// If either PF4(SW1) or PF0(SW2) is pressed, invert the output
		if (!(GPIO_PORTF_DATA_R & 0x10) || !(GPIO_PORTF_DATA_R & 0x01)) {	
			Led = Led^0x02;             // toggle red LED
			GPIO_PORTF_DATA_R &= ~0x02; // clear PF1 
			GPIO_PORTF_DATA_R |= Led;   // output
      pressed_down_flag = 1;			// Flag to indicate either SW1 or SW2 is pressed
			change_flag = 1;            // output PF1 changed
		}
		else if (Led & 0x02) {        // PF4 and PF0 are not pressed and LED is on
			Led &= ~0x02;               // LED (PF1) should be off
			GPIO_PORTF_DATA_R &= ~0x02; // clear PF1 
			pressed_down_flag = 0;
			change_flag = 1;            // output PF1 changed
		}
		else if ((!(Led & 0x02)) && pressed_down_flag == 1) {                           
			pressed_down_flag = 0;      // Led is off and finger has just stopped pressing SW1 and SW2
			change_flag = 1;            // either inputs SW1 or SW2 changed
		}
		else {
			// Do nothing
		}
		
		// Record PortF bits 4,1,0 every time the input or output changes.
		if ((change_flag == 1) && (i < 50)) {
