...
//   Global Variables
unsigned long SW1; // input from PF4, the AS
...
int main(void){
  ...
  while(1){          // Follows the nine steps listed above
...
// Outputs: None
void WaitForASLow(void){
// AS is SW1 (PF4) and negative logic. PF4=0 when pressed, PF4=1 when not pressed
	do {
		SW1 = GPIO_PORTF_DATA_R & 0x10; // Retrieve the value of PF4		
	} while(SW1 == 0x10);	            // While SW is not pressed, keep looping
}
...
// AS is SW1 (PF4) and negative logic. PF4=0 when pressed, PF4=1 when not pressed
	do {
		SW1 = GPIO_PORTF_DATA_R & 0x10; // Retrieve the value of PF4		
	} while(SW1 != 0x10);	            // While SW is pressed, keep looping
}
